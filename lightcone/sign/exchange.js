...
import ABI from "../ethereum/contracts/index.js";
...
export function signFlexCancel(request, keyPair) {
  ...
  // console.log(`signFlexCancel.request`, request)
  let params = `accountId=${request.accountId}`;

  if (request.clientOrderId) params = params + `&clientOrderId=${request.clientOrderId}`;
  if (request.orderHash) params = params + `&orderHash=${request.orderHash}`;
  // console.log(`signFlexCancel.params`, params)

  const encodedParams = encodeURIComponent(params);
  const message = `${method}&${uri}&${encodedParams}`;
  console.log(`signFlexCancel.message`, message)
  const hash = fm.addHexPrefix(sha256(message).toString());
  // console.log(`signFlexCancel.hash`, hash)

  // Create signature
  request.signature = EdDSA.sign(keyPair.secretKey, hash);
  console.log(`signFlexCancel.request.signature`, request.signature)
  ...
  console.log(`signFlexCancel.request.signatureSuccess`, success ? "YES" : "NO")
